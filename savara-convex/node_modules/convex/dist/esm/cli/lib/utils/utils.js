...
export function retryDelay(attempt, _error, _response) {
  const delay = attempt === 0 ? 1 : 2 ** (attempt - 1) * 1e3;
  const randomSum = delay * 0.2 * Math.random();
  return delay + randomSum;
}
function deploymentFetchRetryOn(onError, method, maxRetries = MAX_RETRIES, shouldRetryOverride) {
  const defaultShouldRetry = function(_attempt, error, response) {
    if (error !== null) {
  ...
  const shouldRetry = shouldRetryOverride ? function(_attempt, error, response) {
    return shouldRetryOverride(error, response) ? {
      kind: "retry",
      error: error ?? `Received response with status ${response?.status}`
    } : { kind: "stop" };
  } : defaultShouldRetry;
