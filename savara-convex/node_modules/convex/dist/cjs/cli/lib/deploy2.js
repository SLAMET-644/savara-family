...
var import_evaluateSchema = require("./deployApi/evaluateSchema.js");
...
async function evaluateSchema(ctx, span, request, options, bestEffort) {
  const response = await pushCode(
    ctx,
    span,
    request,
  ...
  return import_evaluateSchema.evaluateSchemaResponse.parse(response);
}
const BEST_EFFORT_RETRIES = 5;
async function pushCode(ctx, span, request, options, endpoint, bestEffort) {
  ...
  const fetch = (0, import_utils.deploymentFetch)(ctx, {
    ...
    ...bestEffort ? {
      ...
      shouldRetry: (error, response) => error !== null || response !== null && response.status >= 500
    } : {}
  });
  try {
